...
Districts are upgraded one at a time from the generic placeholder to a full field guide. 40 are built so far (N°01 Satkhira → N°40 Sylhet); the authoritative, folio-ordered list is `BUILT_DISTRICTS` in `lib/field-guide-folio.ts`. Two divisions are now fully complete: Rangpur (8/8) and Mymensingh (4/4). The workflow:

0. **Pre-flight slug check** — before building, confirm the data slug matches `slugify(GeoJSON ADM2_EN)` for that division (`public/data/divisions/<division>.json`). A mismatch silently breaks the page: `getDistrictData` is an exact key lookup (no fuzzy match) and `DISTRICT_TO_DIVISION` misses, so the field guide never loads and the division falls back to the hardcoded "Dhaka" default. (This bit us with the misspelled `bagherhat` slug — fixed by renaming everything to `bagerhat`.) Also: Chittagong-division overrides must use `division: "Chittagong"` (the `DIVISION_COLORS`/`DISTRICT_TO_DIVISION` key), **not** "Chattogram", or `getThematicColor` falls through to the default blue palette.
