...
Districts are upgraded one at a time from the generic placeholder to a full field guide. 39 are built so far (N°01 Satkhira → N°39 Netrokona); the authoritative, folio-ordered list is `BUILT_DISTRICTS` in `lib/field-guide-folio.ts`. Two divisions are now fully complete: Rangpur (8/8) and Mymensingh (4/4). The workflow:
