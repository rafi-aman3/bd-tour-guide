...
Districts are upgraded one at a time from the generic placeholder to a full field guide. 41 are built so far (N°01 Satkhira → N°41 Moulvibazar); the authoritative, folio-ordered list is `BUILT_DISTRICTS` in `lib/field-guide-folio.ts`. Three divisions are now fully complete: Rangpur (8/8), Mymensingh (4/4) and Sylhet (4/4). The workflow:
