...
Districts are upgraded one at a time from the generic placeholder to a full field guide. 46 are built so far (N°01 Satkhira → N°46 Bhola); the authoritative, folio-ordered list is `BUILT_DISTRICTS` in `lib/field-guide-folio.ts`. Four divisions are now fully complete: Rangpur (8/8), Mymensingh (4/4), Sylhet (4/4) and Khulna (10/10). The workflow:
