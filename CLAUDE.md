...
Districts are upgraded one at a time from the generic placeholder to a full field guide. 28 are built so far (N°01 Satkhira → N°28 Rajshahi); the authoritative, folio-ordered list is `BUILT_DISTRICTS` in `lib/field-guide-folio.ts`. The workflow:
